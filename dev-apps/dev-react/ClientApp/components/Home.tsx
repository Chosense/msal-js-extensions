...
export class Home extends React.Component<RouteComponentProps<{}>, IHomeState> {
    ...
    public render() {
        var usr = this.state.currentUser;

        var loginElem: JSX.Element = usr ?
            <p>
                <button type="button" className="btn btn-primary btn-lg" onClick={ this.onLogout.bind(this) }><span className='glyphicon glyphicon-log-out'></span> Log out</button>
            </p>
            :
            <p>
                <button type="button" className="btn btn-primary btn-lg" onClick={ this.onLogin.bind(this) }><span className='glyphicon glyphicon-log-in'></span> Log in</button>
            </p>;
    ...
    private onLogin(): Promise<User> {
        return MsalService.current.tryGetUserPopup()
            .then(usr => {
